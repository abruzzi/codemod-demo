...
const addPropWhen =
  (
    ...
    prop: string,
    defaultValue: any,
    when: (j: JSCodeshift, root: ASTPath<JSXElement>) => boolean
  ) =>
  (j: JSCodeshift, root: Collection) => {
    ...
    root.findJSXElements(defaultSpecifier).forEach((element) => {
      const node = j.jsxAttribute(
...
const replaceImportStatementFor =
  (pkg: string, convertMap: any) => (j: JSCodeshift, root: Collection) => {
    root
      .find(j.ImportDeclaration)
      .filter((path) => path.node.source.value === pkg)
      .forEach((path) => {
